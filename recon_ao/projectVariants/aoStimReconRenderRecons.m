...
sizeStrLen = length(sizeStr);  %strncmp(sizeStr,rrf.wrapDirInfo(i).name,sizeStrLen)
conditionIndex = 1;
for i = firstGoodDirEntry:length(rrf.wrapDirInfo)
    % Go into top level directories
    if (rrf.wrapDirInfo(i).isdir)

        % Set the directory name and get the list of output directories.
        % These output directories vary in stimulus (color and size) as
        % well as the quads, but are for the optics and monitors fixed.
        ...
        for j = firstGoodDirEntry:length(rrf.mainDirInfo)
            ...
            if rrf.mainDirInfo(j).isdir
                ...
                theName{conditionIndex} = rrf.mainDirInfo(j).name;
                theData(conditionIndex) = load(fullfile(rrf.outputDir, 'xRunOutput.mat'), ...
                    "pr", "cfvRecon", "cfvStim", "stimulusImageLinear", "reconImageLinear", ...
                    "idxXRange", "idxYRange" ...
                    );
...
nConditions = conditionIndex - 1;

%% Color correct 
viewingDisplayScaleFactor = 1;
inputImageScaleFactor = 1;
displayGammaBits = theData(1).pr.displayGammaBits;
displayGammaGamma = theData(1).pr.displayGammaGamma;
maxLinearVal = -Inf;
for cc = 1:nConditions
    % Need to find idxXRange, idxYRange
    cfvStim(cc) = correctForViewing(theData(cc).stimulusImageLinear, rrf.startDisplayName, ...
        rrf.viewingDisplayName, viewingDisplayScaleFactor, rrf.aoReconDir, ...
        displayGammaBits, displayGammaGamma, [], ...
        inputImageScaleFactor, theData(cc).idxXRange, theData(cc).idxYRange);
    if (max(cfvStim(cc).imageRGBNoGamma(:)) > maxLinearVal)
        maxLinearVal = max(cfvStim(cc).imageRGBNoGamma(:));
    end
    cfvRecon(cc) = correctForViewing(theData(cc).reconImageLinear, rrf.startDisplayName, ...
        rrf.viewingDisplayName, viewingDisplayScaleFactor, rrf.aoReconDir, ...
        displayGammaBits, displayGammaGamma, [], ...
        inputImageScaleFactor, theData(cc).idxXRange, theData(cc).idxYRange);
    if (max(cfvRecon(cc).imageRGBNoGamma(:)) > maxLinearVal)
        maxLinearVal = max(cfvRecon(cc).imageRGBNoGamma(:));
    end
end
