...
% Numbers used come from Tuten 2021 ARVO abstract as 
stimSizeDegsList = [1 11 24] / 60; % [2.4 13.2 24]
...
forwardAORenderList = [true false];
reconAORenderList = [true false];
...
forwardChromList = ["chromAllL", "chromAllM", "chromNorm"];
reconChromList = ["chromAllL", "chromAllM", "chromNorm"];
    ...
    for cc = 1:length(stimRValList)
        ...
        for ff = 1:length(forwardDefocusDioptersList)
            ...
            for rr = 1:length(regParaList)
                ...
                for aa = 1:length(forwardAORenderList)
                    forwardAORender = forwardAORenderList(aa);
                    reconAORender = reconAORenderList(aa);
                    for dd = 1:length(forwardChromList)
                        forwardChrom = forwardChromList(dd);
                        reconChrom = reconChromList(dd);
                        for yy = 1:length(slideList)
                            slide = slideList(yy);
                            aoStimRecon_stimSize(displayName,sparsePriorStr,...
